<template>
  <span class="d-flex  text-center" v-if="rowData.Estatus != 'Afiliado'">
    ...
    <vuestic-modal :show.sync="show" ref="activateModal" v-bind:small="true" :cancelClass="'btn btn-secondary btn-micro'"
                  :okClass="'btn btn-primary btn-micro'" :okText="'Confirmar'" :cancelText="'Cancelar'" :okParams="rowData.id" :okAction="activateColmado">
      <div slot="title"><b>Activar colmadero</b></div>
      <div>
        <form-colmado :colmado="rowData"></form-colmado>
      </div>
    ...
    <vuestic-modal :show.sync="show" ref="binModal" v-bind:small="true" :cancelClass="'btn btn-secondary btn-micro'"
                    :okClass="'btn btn-warning btn-micro'" :okText="'Confirmar'" :cancelText="'Cancelar'" :okParams="rowData.id"  :okAction="discardColmado">
...
export default {
  ...
  methods: {
    ...
    activateColmado (user_id) {
      ...
      axios.put('https://api2.socialandloyal.com/users/user_id/' + user_id, params,
        {
          headers: {
